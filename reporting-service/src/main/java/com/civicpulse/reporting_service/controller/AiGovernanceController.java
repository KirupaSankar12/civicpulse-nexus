...
public class AiGovernanceController {
    ...
    public ResponseEntity<AiGovernanceResponse> analyze() {
        AiGovernanceResponse response = geminiService.analyzeGovernance();
     ...
     */
    @PostMapping("/chat")
    public ResponseEntity<AiGovernanceResponse> chat(@RequestBody AiGovernanceRequest request) {
        AiGovernanceResponse response = geminiService.chat(request.getQuestion());
        return ResponseEntity.ok(response);
    }
}
